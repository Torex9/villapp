...
export const MobileNav: React.FC<MobileNavProps> = ({ nav, open }) => {
  ...
  return (
    <>
      ...
      >
        <PopoverPanel className="fixed right-0 top-0 z-20 flex h-full min-h-screen w-[85vw] max-w-[24rem] shadow-md">
              ...
              <div className="flex w-full justify-end px-3">
                <PopoverButton
                  className={clsx(
                    ...
                    'flex shrink-0 items-center justify-center duration-150 hover:text-white'
                  )}
                >
                  <X />
                </PopoverButton>
                        ...
                        <Disclosure as="div">
                          {({ open }) => (
                            <>
                              {item.url && (
                                <DisclosureButton
                                  className={clsx(
                                    ...
                                    activeLink(item.url) || doesUrlMatch(item.urlMatcher, pathname)
                                      ? 'pl-4 text-white'
                                  ...
                                  <span className="flex items-center">
                                    <ChevronDown
                                      className={clsx(
                                        ...
                                        activeLink(item.url) ? 'text-white' : 'text-alpha-light-400',
                              ...
                              <DisclosurePanel as="div" className="bg-alpha-dark-100">
                                <ul>
                                  {item.children?.map(
                                    (child) =>
                                      child.url && (
                                        <li key={child.name} className="border-b border-white/20">
                                          <PopoverButton as={Link} href={child.url}>
                                            <div
                                              className={clsx(
                          ...
                          {item.url && (
                            <Link
                              ...
                              className={clsx(
                                ...
                                activeLink(item.url) ? 'pl-4 text-white' : 'pl-3 text-alpha-light-400'
                              )}
                            >
                              {item.name}
                            </Link>
